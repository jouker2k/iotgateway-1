...
class Receiver(SubscribeCallback):
    ...
    def message(self, pubnub, message):
        ...
        try: # try to set cipher key then re-read message
            msg = message.message
            ...
            while type(msg) != dict:
                    if user_uuid is not None:
                        ...
                        envelope = pubnub.history().channel(message.channel).count(1).reverse(True).sync() # re-read
        ...
        if message.channel == self.admin_channel and "success" not in msg and "error" not in msg: # install modules remotely
